<template>
  <div class="about">
    ...
    <div class="section">
      ...
      <div>
        Принимает флаг темной темы (влияет на цвет текста в лого, пунктов меню
        верхнего порядка, также на цвет изображения бренда при включенном флаге
        monochrome-brand-image), строчное наименование темы (влияет только на
        цвет фона панели), флаг прокручиваемости (размер прокручиваемой области
        захардкожен), флаг одноцветности изображения бренда, флаг
        несворачивоемости панели (true - никогда не сворачивается, false -
        всегда сверута, при использовании Offcanvas-панели данный флаг должен
        быть установлен в true), наименование точки, на котором панель
        разворачивается (в сочетании с expand), объект бренда, флаг отсутствия
        коллапс-кнопки, строчное значение выравнивания контента. Список пунктов
        навигации и остальные элементы пробрасываются в слот.
      </div>
      <pre>
props: {
  dark: Boolean,
